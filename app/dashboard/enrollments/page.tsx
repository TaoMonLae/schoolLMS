...
export default async function EnrollmentsPage({ searchParams }: EnrollmentsPageProps) {
  ...
  return <div className="space-y-6 pb-10"><PageHeader eyebrow="Enrollments" title="Enrollment Tracking" description="Connect students to classes with tenant-scoped enrollment histories." />
    ...
    {canManage ? <form action={enrollStudent} className="grid gap-3 rounded-lg border border-hairline bg-canvas p-4 shadow-soft md:grid-cols-3"><select name="studentId" className="rounded-md border border-hairline px-3 py-2 text-sm">{students.map(s=><option key={s.id} value={s.id}>{s.studentNumber} — {s.preferredName || s.legalName}</option>)}</select><select name="classId" className="rounded-md border border-hairline px-3 py-2 text-sm">{classes.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select><button className="rounded-md bg-primary px-4 py-2 text-sm font-bold text-on-primary">Enroll / transfer</button></form> : null}
