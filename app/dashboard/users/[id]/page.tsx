...
export default async function UserDetailPage({ params, searchParams }: UserDetailProps) {
  ...
  return (
    <div className="space-y-6 pb-10">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><PageHeader eyebrow="Users & Access" title={user.name} description={user.email} /><Link href={`/dashboard/users/${user.id}/edit`} className="rounded-md bg-ink px-4 py-3 text-sm font-bold text-on-dark hover:bg-slate">Edit User</Link></div>
      ...
      <section className="grid gap-5 lg:grid-cols-2">
        <form action={resetPasswordAction} className="rounded-lg border border-hairline bg-canvas p-5 shadow-soft"><input type="hidden" name="id" value={user.id} /><h2 className="text-lg font-semibold text-ink">Reset Password</h2><input name="password" type="password" placeholder="Temporary password" className="mt-4 w-full rounded-md border border-hairline bg-canvas px-3 py-3 text-sm text-ink" /><button className="mt-4 rounded-md bg-ink px-4 py-3 text-sm font-bold text-on-dark">Set Temporary Password</button></form>
