  ...
  return (
    <div className="space-y-6 pb-10">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        {canUpload ? (
          <Link href="/dashboard/videos/new" className="inline-flex shrink-0 items-center justify-center gap-2 rounded-md bg-ink px-4 py-3 text-sm font-bold text-white hover:bg-moss">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {lessons.map((lesson) => (
          <article key={lesson.id} className="rounded-lg border border-line bg-white p-4 shadow-soft">
            <VideoThumbnail lesson={lesson} />
            <div className="mt-4">
              <p className="text-xs font-semibold uppercase tracking-wide text-clay">{lesson.subjectName} | {lesson.className}</p>
              <h2 className="mt-2 text-lg font-semibold leading-6 text-ink">{lesson.title}</h2>
              <p className="mt-2 line-clamp-2 text-sm leading-6 text-moss">{lesson.description}</p>
              <div className="mt-4 flex flex-wrap gap-2">
                <span className="rounded-md bg-rice px-2 py-1 text-xs font-semibold text-moss">{formatEnumLabel(lesson.videoProvider)}</span>
                <span className="rounded-md bg-rice px-2 py-1 text-xs font-semibold text-moss">{lesson.durationMinutes || 0} min</span>
                <span className="rounded-md bg-rice px-2 py-1 text-xs font-semibold text-moss">{formatEnumLabel(lesson.visibility)}</span>
              </div>
              <Link href={`/dashboard/videos/${lesson.id}`} className="mt-5 inline-flex w-full justify-center rounded-md bg-ink px-4 py-2 text-sm font-bold text-white hover:bg-moss">
                Watch Lesson
              </Link>
            </div>
          </article>
      ...
      {lessons.length === 0 ? <div className="rounded-lg border border-line bg-white p-8 text-center text-sm text-moss shadow-soft">No video lessons match the current filters.</div> : null}
    </div>
  );
}
