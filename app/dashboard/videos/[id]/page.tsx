        ...
        <section className="overflow-hidden rounded-lg border border-hairline bg-canvas shadow-soft">
          ...
          <div className="p-5">
            <div className="flex flex-wrap gap-2">
              <span className="rounded-md bg-surface px-3 py-2 text-xs font-semibold text-slate">{formatEnumLabel(lesson.videoProvider)}</span>
              <span className="rounded-md bg-surface px-3 py-2 text-xs font-semibold text-slate">{formatEnumLabel(lesson.visibility)}</span>
              <span className="rounded-md bg-surface px-3 py-2 text-xs font-semibold text-slate">Uploaded by {lesson.uploadedBy}</span>
            </div>
            <p className="mt-4 max-w-3xl text-sm leading-6 text-slate">{lesson.description}</p>
          </div>
        </section>
            ...
            {playlist.map((playlistLesson, index) => (
              <Link
                ...
                className={`block rounded-md border p-3 ${
                  playlistLesson.id === lesson.id ? "border-primary bg-tint-yellow" : "border-hairline bg-surface hover:bg-canvas"
                }`}
              >
                <p className="text-xs font-semibold uppercase tracking-wide text-slate">Lesson {index + 1} | {playlistLesson.durationMinutes || 0} min</p>
                <p className="mt-1 text-sm font-semibold leading-5 text-ink">{playlistLesson.title}</p>
                <p className="mt-1 text-xs text-slate">{formatEnumLabel(playlistLesson.videoProvider)}</p>
              </Link>
            ))}
          </div>
        </aside>
              ...
              <div key={student?.id || item.progress?.id} className="flex flex-col gap-3 py-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate">{watchedMinutes} minutes watched</p>
                  </div>
                </div>
                <span className={`rounded-md px-3 py-2 text-xs font-semibold ${completed ? "bg-tint-mint text-success" : item.progress ? "bg-surface text-slate" : "bg-tint-rose text-error"}`}>
                  {completed ? "Completed" : item.progress ? "In progress" : "Not watched"}
                </span>
              </div>
            );
          })}
          {(canSeeClassProgress ? classProgress : progress).length === 0 ? <div className="py-6 text-sm text-slate">No progress has been recorded for this lesson yet.</div> : null}
        </div>
      </section>
    </div>
  );
