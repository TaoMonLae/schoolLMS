...
export default async function EditClassPage({ params, searchParams }: { params: Promise<{ classId: string }>; searchParams?: Promise<{ error?: string }> }) {
  ...
  if (!klass) notFound();
  return <div className="space-y-6 pb-10"><PageHeader eyebrow="Classes" title="Edit Class" description="Update class details and teacher assignment." />{query?.error ? <div className="rounded-lg border border-error/30 bg-tint-rose p-4 text-sm font-semibold text-error">{query.error}</div> : null}<form action={updateClass} className="grid gap-4 rounded-lg border border-hairline bg-canvas p-5 shadow-soft sm:grid-cols-2"><input type="hidden" name="id" value={klass.id} /><Input name="name" label="Class name" defaultValue={klass.name} required /><Input name="academicYear" label="Academic year" defaultValue={klass.academicYear} required /><Input name="gradeLevel" label="Grade level" defaultValue={klass.gradeLevel || ""} /><Input name="room" label="Room" defaultValue={klass.room || ""} /><label className="text-sm font-semibold text-ink">Teacher<select name="teacherId" defaultValue={klass.teacherId || ""} className="mt-2 w-full rounded-md border border-hairline px-3 py-3 text-sm"><option value="">No teacher</option>{teachers.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}</select></label><div className="sm:col-span-2"><button className="rounded-md bg-primary px-5 py-3 text-sm font-bold text-on-primary">Save Class</button></div></form></div>;
}
