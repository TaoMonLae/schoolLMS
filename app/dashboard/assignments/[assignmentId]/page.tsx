            ...
            <tbody className="divide-y divide-line">
              {rows.map(({ student, submission }) => (
                <tr key={student.id}>
                  ...
                  <td className="px-4 py-3">{canGrade ? <input name={`points_${student.id}`} type="number" min={0} max={assignment.maxPoints} defaultValue={submission?.points ?? undefined} placeholder="--" className="w-20 rounded-md border border-line px-2 py-1" /> : submission?.points ?? "-"}</td>
                  <td className="px-4 py-3">{canGrade ? <input name={`feedback_${student.id}`} defaultValue={submission?.feedback ?? undefined} placeholder="Feedback" className="w-full rounded-md border border-line px-2 py-1" /> : submission?.feedback || "-"}</td>
                </tr>
              ))}
            </tbody>
          </table>{canGrade ? <button className="mt-4 rounded-md bg-ink px-4 py-2 text-sm font-bold text-white">Save grades</button> : null}
        </form>
      </section>
    </div>
  );
