...
export default async function EditSubjectPage({ params }: { params: Promise<{ subjectId: string }> }) {
  ...
  if (!subject) notFound();
  return <div className="space-y-6 pb-10"><PageHeader eyebrow="LMS" title="Edit Subject" description="Update a GED/custom subject." /><form action={updateSubject} className="grid gap-4 rounded-lg border border-line bg-white p-5 shadow-soft sm:grid-cols-2"><input type="hidden" name="id" value={subject.id} /><Input name="name" label="Name" defaultValue={subject.name} required /><Input name="code" label="Code" defaultValue={subject.code || ""} /><label className="flex items-center gap-2 text-sm font-semibold text-ink"><input type="checkbox" name="isGed" defaultChecked={subject.isGed} /> GED subject</label><label className="sm:col-span-2 text-sm font-semibold text-ink">Description<textarea name="description" defaultValue={subject.description || ""} rows={5} className="mt-2 w-full rounded-md border border-line px-3 py-3 text-sm" /></label><div className="sm:col-span-2"><button className="rounded-md bg-ink px-5 py-3 text-sm font-bold text-white">Save Subject</button></div></form><form action={deleteSubjectSafely}><input type="hidden" name="id" value={subject.id} /><button className="rounded-md border border-[#8b2b20] px-4 py-2 text-sm font-bold text-[#8b2b20]">Delete if unused</button></form></div>;
}
