...
export default async function SchoolsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {schools.length === 0 ? (
        <div className="rounded-lg border border-line bg-white px-5 py-16 text-center shadow-soft">
          <Building2 className="mx-auto h-9 w-9 text-moss/40" />
          <p className="mt-3 text-sm font-semibold text-ink">No schools yet</p>
          <p className="mt-1 text-sm text-moss">Create the first school to get started.</p>
          <Link
            href="/super-admin/schools/new"
        ...
        <div className="overflow-hidden rounded-lg border border-line bg-white shadow-soft">
          <table className="w-full text-sm">
            <thead className="border-b border-line bg-rice">
              <tr>
                <th className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-moss">
                  School
                </th>
                <th className="hidden px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-moss lg:table-cell">
                  Domain / Subdomain
                </th>
                <th className="px-5 py-3 text-right text-xs font-semibold uppercase tracking-wide text-moss">
                  Students
                </th>
                <th className="hidden px-5 py-3 text-right text-xs font-semibold uppercase tracking-wide text-moss sm:table-cell">
                  Classes
                </th>
                <th className="hidden px-5 py-3 text-right text-xs font-semibold uppercase tracking-wide text-moss sm:table-cell">
                  Users
            ...
            <tbody className="divide-y divide-line">
              {schools.map((school) => {
                ...
                return (
                  <tr key={school.id} className="hover:bg-rice/60">
                    ...
                    <td className="hidden px-5 py-4 lg:table-cell">
                      {domain ? (
                        <span className="inline-flex items-center gap-1 rounded-md bg-rice px-2 py-1 text-xs font-mono text-moss">
                          {domain}
                          <ExternalLink className="h-3 w-3" aria-hidden="true" />
                        </span>
                      ) : (
                        <span className="text-xs text-moss/50">—</span>
                      )}
                    </td>
                    <td className="px-5 py-4 text-right font-semibold text-ink">
                      {school._count.students}
                    </td>
                    <td className="hidden px-5 py-4 text-right text-moss sm:table-cell">
